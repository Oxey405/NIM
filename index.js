...
if(os.platform() == "darwin") {
    console.log("⚠MacOS version has NOT been tested yet. Please provide feedback and feel free to report an issue at https://github.com/Oxey405/NIM")
}
//remove this if not in BETA
console.log("⚠This software is still in Beta. Feel free to report an issue at https://github.com/Oxey405/NIM or contact by email at Oxey405@protonmail.com")
let storedVersions = [];
if(!fs.existsSync(versionStoreDir)) {
    try {
        fs.mkdirSync(versionStoreDir);
    } catch (error) {
        console.error("❌Critical error : could not create the folder " + versionStoreDir + ". Does NIM has enough privileges ?")
    }
} else {
...
args.forEach(arg => {
    ...
    if(arg.startsWith("mode=")) {
        ...
        if(appMode != "install" && appMode != "switch" && appMode != "download") {
            console.log("❌ Critical error : argument \"mode\" was set to " + appMode + " expected install, switch or remove")
        }
    }
    ...
    if(arg.startsWith("download=")) {
        ...
        if(downloadMode != "normal" && downloadMode != "force") {
            console.log("❌Download mode was set to " + downloadMode + " expected normal or force")
        }
    }
    ...
    if(arg == "help" || arg == "doc") {
        ...
        console.log("nodeVersion=(latest/vX.Y.Z) | set the node version to manage/install | default : latest")
        console.log("mode=(install/switch/remove) | Install, switch to or remove a version of NodeJS | default : install")
        console.log("download=(normal/force) | Force the downloading from source or use version caching | default : normal")
        console.log("\r\nStoring version system (or version caching) :\r\n")
        console.log("If a version is already stored in the version storage found at " + versionStoreDir + ".\r\nIt won't download unless you set download to force (download=force)")
        console.log("\r\nIntent of this software (or what is the use of NIM)\r\n")
        console.log("NIM is here to install and manage nodeJS versions. It require the user to have at least one version of NodeJS installed \r\n or else it won't be able to run properly.")
...
.then(res => res.json())
.then(body => {
    console.log("✅Got the versions ! Checking availability for : " + targettedVersion)
...
function downloadVersion() {
    if(!versions.includes(targettedVersion)) {
        ...
        console.log("📥Version available, downloading for " + sysOS + " architecture " + sysArch);
        let fileType;
        let sysOSName = sysOS;
        if(sysOS == "linux") {
            fileType = "tar.gz"
        ...
        let downloadURL = downloadVersionURLStart + targettedVersion + "/node-" + targettedVersion + "-" + sysOSName + "-" + sysArch + "." + fileType;
        console.log("Downloading from : " + downloadURL + " in 3 seconds. Press Ctrl+C to cancel.");
        setTimeout(() => {
...
function downloadAndInstallVersion(url, fileType) {
    ...
    if(urlToDownload.hostname !== "nodejs.org") {
        ...
        let count = 0;
        let dlFilePath = (tmpDir + `node-${targettedVersion}-${sysOS}-${sysArch}.${fileType}`);
        ...
        https.get(urlToDownload.href, function(response) {
            ...
            console.log("downloading ~" + Math.round(response.headers[ 'content-length' ] / 10**6) + " Megabytes (Mo) of data")
            downloadedFile.on("finish", () => {
                downloadedFile.close();
                console.log(Math.round(response.headers[ 'content-length' ] / 10**6) + "/" + Math.round(response.headers[ 'content-length' ] / 10**6) + " Mo")
                fs.renameSync(dlFilePath, (versionStoreDir + "/" + targettedVersion + "." + fileType));
                console.log("💾Download complete and stored ! Installing in 2 seconds")
                storedVersions.push(targettedVersion);
                setTimeout(() => {enableVersion(targettedVersion)}, 2000)
            })
            ...
            downloadedFile.on("drain", () => {
                count++;
                if(count >= 200) {
                    console.log(Math.round(downloadedFile.bytesWritten / 10**6) + "/" + Math.round(response.headers[ 'content-length' ] / 10**6) + " Mo")
                    count = 0;
                }
            })
...
async function enableVersion(version) {
    if(!storedVersions.includes(version)) {
        console.error("❌Could not enable version " + version + " reason : This version isn't stored.");
        return;
    }
    let fileType;
    if(sysOS == "linux") {
